...
export class AppSidebarComponent implements OnInit, OnDestroy {
  ...
  filteredOthersItems: NavItem[] = [];

  // Menu principal avec rôles
 ...
 roles: ['Admin'] ,  icon: `<svg width="1em" height="1em" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    // }, 
    {
      ...
      icon: `<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
               <path stroke-linecap="round" stroke-linejoin="round" d="M17 20h5v-2a4 4 0 0 0-4-4h-1M9 20H4v-2a4 4 0 0 1 4-4h1m0-4a4 4 0 1 1 8 0 4 4 0 0 1-8 0z"/>
             </svg>`,
       ...
       subItems: [
 {
...

    {
      ...
      icon: `<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
               <path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6m2 4H7a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v12a2 2 0 01-2 2z"/>
             </svg>`,
      path: "/tpes",
      roles: ['Admin', 'Commercant'], // Admin voit tous, Commercant voit ses TPEs
    },
    {
      ...
      name: "Incidents",
      path: "/incidents",
      roles: ['Admin', 'Commercant'], // Admin voit tous, Commercant voit ses incidents
    },
    {
      name: "Tickets",
      icon: `<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
      ...
      path: "/tickets",
      roles: ['Admin', 'Technicien'], // Admin voit tous, Technicien voit ses tickets assignés
    },
    // Dans votre app-sidebar.component.ts, ajoutez dans allNavItems :

{
  ...
  name: "Archives",
  path: "/archives",
  roles: ['Admin', 'Technicien', 'Commercant'], // Tous peuvent voir leurs archives
},
    // {
    //   name: "Forms",
    ...
    //   roles: ['Admin'],
    //   subItems: [
    //     { name: "Basic Tables", path: "/basic-tables", pro: false, roles: ['Admin'] },
    //   ],
    // },
    // {
    //   name: "Pages",
    ...
    //   roles: ['Admin'],
    //   subItems: [
    //     { name: "Blank Page", path: "/blank", pro: false, roles: ['Admin'] },
    //     { name: "404 Error", path: "/error-404", pro: false, roles: ['Admin'] },
    //   ],
    // },
  ];

  // Menu Autres avec rôles
  private allOthersItems: NavItem[] = [
    // {
    ...
    //   roles: ['Admin'],
    //   subItems: [
    //     { name: "Line Chart", path: "/line-chart", pro: false, roles: ['Admin'] },
    ...
    //     { name: "Badge", path: "/badge", pro: false, roles: ['Admin'] },
    //     { name: "Buttons", path: "/buttons", pro: false, roles: ['Admin'] },
    //     { name: "Images", path: "/images", pro: false, roles: ['Admin'] },
    //     { name: "Videos", path: "/videos", pro: false, roles: ['Admin'] },
    //   ],
    // },
    // {
    ...
    //   roles: ['Admin'],
    //   subItems: [
    //     { name: "Sign In", path: "/signin", pro: false, roles: ['Admin'] },
    //     { name: "Sign Up", path: "/signup", pro: false, roles: ['Admin'] },
    //   ],
    // },
  ];
  ...
  ngOnInit() {
    ...
    this.userService.getMyProfile().subscribe({
      ...
      error: (err) => {
        ...
        // En cas d'erreur, on filtre avec un rôle par défaut ou on vide le menu
        this.userRole = '';
        this.filterMenuByRole();
      }
    });
  ...
  private filterMenuByRole() {
    this.filteredNavItems = this.filterItemsByRole(this.allNavItems);
    this.filteredOthersItems = this.filterItemsByRole(this.allOthersItems);
  }

  private filterItemsByRole(items: NavItem[]): NavItem[] {
    return items
      .map(item => {
        // Vérifier si l'item principal est accessible
        const itemAccessible = !item.roles || item.roles.includes(this.userRole);
        ...
        if (item.subItems) {
          ...
          // Si après filtrage il n'y a plus de sous-items, on ne garde pas l'item principal
