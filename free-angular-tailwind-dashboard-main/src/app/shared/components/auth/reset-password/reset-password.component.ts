...
export class ResetPasswordComponent implements OnInit, OnDestroy {
      ...
      .subscribe({
        next: (response) => {
          ...
          if (response && response.resultCode === 0) {
            this.showSuccessMessage(response.message || 'Mot de passe réinitialisé avec succès ! Redirection vers la connexion...');
  ...
  resendOtp(): void {
    if (!this.email) {
      this.showErrorMessage('Email non disponible pour renvoyer le code');
      return;
    }
      ...
      .subscribe({
        next: (response) => {
          if (response && (response.resultCode === 0 || response.resultCode === 1)) {
            this.showSuccessMessage('Un nouveau code OTP a été envoyé à votre email.');
          } else {
            this.showErrorMessage(response?.message || 'Échec de l\'envoi du code OTP.');
          }
        },
        error: (error) => {
          this.showErrorMessage(error.message || 'Erreur lors de l\'envoi du nouveau code.');
        }
      });
  }
